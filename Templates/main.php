...
$myCarousel = new Carousel($pdo);
$getData = $myCarousel->getCarousel();

?>
<main>
    <div id="myCarousel" class="carousel slide mb-6" data-bs-ride="carousel">
        <div class="carousel-indicators">
            <?php foreach ($getData as $count => $slide): ?>
                <button
                        type="button"
                        data-bs-target="#myCarousel"
                        data-bs-slide-to="<?= $count ?>"
                        class="<?= $count === 0 ? 'active' : '' ?>"
                        aria-current="<?= $count === 0 ? 'true' : '' ?>">
                </button>
            <?php endforeach; ?>
        </div>
        ...
        <div class="carousel-inner">
            <?php foreach ($getData as $count => $slide): ?>
                <div class="carousel-item <?= $count === 0 ? 'active' : '' ?>">
                    <img class="img-fluid" src="/CarouselImage<?= $slide['image_name'] ?>" alt="<?= $slide['title'] ?>">
                    <div class="container">
                        <div class="carousel-caption text-end">
                            <h1><?= $slide['title'] ?></h1>
                            <p><?= $slide['description'] ?></p>
                            <a class="btn btn-lg btn-primary" href="<?= $slide['link'] ?>">Learn more</a>
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
